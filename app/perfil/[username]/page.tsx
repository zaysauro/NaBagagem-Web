...
export default async function PublicProfile({params}:{params:Promise<{username:string}>}) {
  ...
  return <main className="min-h-screen bg-neutral-50 px-6 py-10"><div className="mx-auto max-w-5xl">
    ...
    <section className="mt-5 rounded-3xl border bg-white p-7 shadow-sm"><div className="flex items-center gap-5">
      ...
      <div><h1 className="text-3xl font-bold">{profile.display_name||"Viajante"}</h1><p className="text-neutral-500">@{profile.username}</p>{profile.bio&&<p className="mt-2 text-neutral-600">{profile.bio}</p>}</div>
    </div></section>
