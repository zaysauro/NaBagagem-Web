...
export async function GET(request: Request) {
  ...
  // Avoid nested profile relations because this database may not have a PostgREST FK from feed_posts to profiles.
  ...
  const normalized = (posts || []).map((p: any) => {
    ...
    return {
      ...p,
      profiles: profiles.get(p.user_id) || null,
      isMine,
      feedSource: source,
      feedRank: rank,
      likes: (likesByPost.get(p.id) || []).length,
      likedByMe: (likesByPost.get(p.id) || []).some((x: any) => x.user_id === user.id),
      comments: postComments.filter((x: any) => x.approved || x.user_id === user.id || p.user_id === user.id).map((x: any) => ({ ...x, profiles: profiles.get(x.user_id) || null })),
      bookmarkedByMe: bookmarks.has(p.id),
  ...
  return NextResponse.json({
    ...
    page,
    pageSize,
    hasMore: normalized.length > from + pageSize || (posts || []).length === candidateSize,
    sources: {
      friends: normalized.filter((p: any) => p.feedSource === "friends").length,
      discover: normalized.filter((p: any) => p.feedSource === "discover").length,
    },
  });
}
