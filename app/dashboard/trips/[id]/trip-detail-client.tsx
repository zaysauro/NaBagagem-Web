...
export default function TripDetailClient({ tripId, initialLocations, initialEvents = [], canEdit = true }: { tripId: string; initialLocations: Location[]; initialEvents?: Event[]; canEdit?: boolean }) {
  ...
  const bufferWarnings = useMemo(() => {
    ...
    for (const [, dayEvents] of groupedEvents) {
      for (let i = 0; i < dayEvents.length - 1; i++) {
        const current = dayEvents[i];
        const next = dayEvents[i + 1];
        if (!current.end_time || !next.start_time || current.event_date !== next.event_date) continue;
        const [eh, em] = current.end_time.slice(0, 5).split(":").map(Number);
        const [sh, sm] = next.start_time.slice(0, 5).split(":").map(Number);
        const gap = (sh * 60 + sm) - (eh * 60 + em);
        if (gap < 0) continue;
        const route = travelInfo[current.location_id + ":" + next.location_id];
        if (route && route.durationMinutes > gap) warnings.add(current.id + ":" + next.id);
      }
    }
    return warnings;
  }, [groupedEvents, travelInfo]);
