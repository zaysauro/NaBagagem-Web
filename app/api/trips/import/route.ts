...
export async function POST(request: Request) {
  ...
  if (name.endsWith(".json")) {
    ...
    const locations = Array.isArray(backup.locations) ? backup.locations.slice(0,500).map((p:any,index:number)=>({trip_id:trip.id,name:String(p.name||"Destino"),city:p.city||null,country:p.country||null,latitude:Number.isFinite(Number(p.latitude))?Number(p.latitude):null,longitude:Number.isFinite(Number(p.longitude))?Number(p.longitude):null,visited_at:p.visited_at||null,notes:p.notes||null,order_index:index})) : [];
    const events = Array.isArray(backup.events) ? backup.events.slice(0,1000).map((e:any)=>({trip_id:trip.id,title:String(e.title||"Atividade"),description:e.description||null,event_date:e.event_date||null,start_time:e.start_time||null,end_time:e.end_time||null,location_id:null,day_index:Number.isInteger(Number(e.day_index))?Number(e.day_index):1,status:["completed","in_progress","future"].includes(e.status)?e.status:"future",color:/^#[0-9a-f]{6}$/i.test(String(e.color||""))?e.color:"#111827",reservation_name:e.reservation_name||null,confirmation_code:e.confirmation_code||null,reservation_url:e.reservation_url||null,reminder_minutes:Number.isInteger(Number(e.reminder_minutes))?Number(e.reminder_minutes):null})) : [];
    ...
    if(locations.length){const {error}=await supabase.from("trip_locations").insert(locations);if(error)return NextResponse.json({error:error.message},{status:400});}
