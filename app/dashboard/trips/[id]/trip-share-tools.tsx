...
export default function TripShareTools({ tripId }: { tripId: string }) {
  ...
  return <section className="mt-5 rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm sm:mt-7 sm:p-6">
    ...
    {shareUrl && <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-start"><div className="min-w-0 flex-1"><input ref={input} readOnly value={shareUrl} onFocus={e => e.currentTarget.select()} className="w-full min-h-11 rounded-xl border bg-neutral-50 px-3 py-2 text-sm" /></div><TripQr shareUrl={shareUrl} /></div>}
