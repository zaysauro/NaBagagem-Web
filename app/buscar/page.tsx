...
export default function SearchPage(){
 const [q,setQ]=useState("");const [results,setResults]=useState<Results>({users:[],posts:[],destinations:[]});const [loading,setLoading]=useState(false);const [searched,setSearched]=useState(false);const [message,setMessage]=useState("");
 async function search(value=q){const term=value.trim();if(!term){setResults({users:[],posts:[],destinations:[]});setSearched(false);return}setLoading(true);setMessage("");const r=await fetch("/api/search?q="+encodeURIComponent(term),{cache:"no-store"});const d=await r.json();if(r.ok)setResults(d);else setMessage(d.error||"Não foi possível buscar.");setLoading(false);setSearched(true);}
 useEffect(()=>{const params=new URLSearchParams(window.location.search);const initial=params.get("q")||"";if(initial){setQ(initial);search(initial)}},[]);
