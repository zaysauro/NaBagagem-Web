...
type Location = { id: string; name: string; city: string | null; country: string | null; visited_at: string | null; notes: string | null };
...
export default function TripDetailClient({ tripId, initialLocations, initialEvents = [] }: { tripId: string; initialLocations: Location[]; initialEvents?: Event[] }) {
  ...
  async function addLocation(e: React.FormEvent) {
    ...
    setLocations((current) => [...current, data.location]); setLocationForm({ name: "", city: "", country: "", visited_at: "", notes: "" });
  }
    ...
    <section className="mt-7 space-y-7">
      <div className="grid gap-7 lg:grid-cols-2">
        <form onSubmit={addLocation} className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-neutral-500">Lugares que fazem parte da viagem.</p>
          <div className="mt-5 space-y-3">
            {([["name","Nome do lugar","ex.: Tokyo"],["city","Cidade","ex.: Tóquio"],["country","País","ex.: Japão"]] as const).map(([key,label,placeholder]) => <label key={key} className="block text-sm font-medium text-neutral-700">{label}<input required={key==="name"} value={locationForm[key]} onChange={(e)=>setLocationForm({...locationForm,[key]:e.target.value})} placeholder={placeholder} className="mt-1 w-full rounded-xl border border-neutral-300 px-3 py-2.5 outline-none focus:border-neutral-950" /></label>)}
          ...
          <button disabled={locationLoading} className="mt-4 w-full rounded-xl bg-neutral-950 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{locationLoading ? "Adicionando..." : "Adicionar destino"}</button>
          <div className="mt-5 space-y-2">{locations.map((item,index)=><div key={item.id} className="flex items-center justify-between rounded-xl bg-neutral-50 p-3"><div><b>{index+1}. {item.name}</b><p className="text-xs text-neutral-500">{[item.city,item.country].filter(Boolean).join(", ")}</p></div><button type="button" onClick={()=>removeLocation(item.id)} className="text-xs font-semibold text-red-600">Remover</button></div>)}</div>
        </form>
