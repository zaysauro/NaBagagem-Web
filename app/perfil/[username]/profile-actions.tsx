...
export default function ProfileActions({ username }: { username: string }) {
  ...
  async function showConnections(type: "followers" | "following") {
    ...
    const r = await fetch(
      "/api/social/connections?username=" +
  ...
  if (!data) {
    return <div className="mt-4 text-sm text-neutral-500">Carregando conexões...</div>;
  }
  ...
  return (
    <div className="mt-5 flex flex-wrap items-center gap-3">
      <button
        ...
        className="text-left text-sm text-neutral-500 hover:underline"
      >
        <b className="text-neutral-900">{data.followers}</b> seguidores
      </button>
      ...
      <button
        ...
        className="text-left text-sm text-neutral-500 hover:underline"
      >
        <b className="text-neutral-900">{data.following}</b> seguindo
      </button>
      ...
      {(listLoading || list) && (
        <div className="w-full rounded-2xl border bg-neutral-50 p-4">
          ...
          {listLoading ? (
            ...
            <div className="mt-3 space-y-2">
              {list.map((u) => (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold">{u.display_name || "Viajante"}</p>
            ...
            <p className="mt-3 text-sm text-neutral-500">Nenhuma conexão encontrada.</p>
          )}
        </div>
      )}
    </div>
