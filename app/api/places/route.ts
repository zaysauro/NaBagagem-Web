...
export async function GET(request: Request) {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng) || !queries[category]) {
  ...
  const places = (data.elements || []).map((item: any) => ({
    id: String(item.type || "poi") + "-" + String(item.id),
